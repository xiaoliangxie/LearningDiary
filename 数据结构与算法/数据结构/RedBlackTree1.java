...
public class RedBlackTree1<T extends Comparable> {
    ...
    public void add(T e) {
        // 如果根节点为空设置为根节点，默认是黑色
        if (root == null) {
            this.root = new RedBlackTree1.Node(e);
        } else {
            // 根节点不为空的情况
            RedBlackTree1.Node current = root;
            RedBlackTree1.Node parent = null;
            int cmp = 0;
            do {
                // 二叉搜索树的查找方法，找到合适的节点
                parent = current;
            ...
            RedBlackTree1.Node newNode = new RedBlackTree1.Node(e, parent, null, null);
            if (cmp > 0) {
    ...
    private void recursive(RedBlackTree1.Node node) {
        if (node.left != null) {
            recursive(node.left);
        }
    ...
    public static void main(String[] args) {
        RedBlackTree1<Integer> tree = new RedBlackTree1<Integer>();
